...
% @Date - 4/26/2022
%% Get Run Name
prompt = {'Enter Name For Run', 'Sample Size (n = ?)', 'Margin','Run Type (Rates Only or vary MMOI)'};
dlgtitle = 'Run Info';
dims = [1 40];
...
run_type = convertCharsToStrings(answers{4});
...
if strcmp(run_type,"MMOI") 
    if margin ~= 0
        margin = 0;
        msgbox('Going to set Margin to zero for MMOI runs. Vary one thing at a time.')
    end
elseif strcmp(run_type,"Rates")
    if(margin < 0)
        msgbox('Going to set Margin to zero, negative margin is not allowed.')
        margin = 0;
    end
else
    msgbox('Currently, acceptable run types are "Rates" or "MMOI". Please enter one of those two options.')
    return
end
...
%A run with just rates variations
if strcmp(run_type,"Rates") 
    for i = 1:n
...
if strcmp(run_type,"MMOI")
    ...
    for i = 1:n
        ...
        data_store.margin = margin;
        data_store.run_type = run_type;
        data_store.MMOI_tensors = MMOI_tensors;
        save(run_name, 'data_store');
        cd ../
